...
extension DateScrollManager {
    ...
    private func previousMonthDays(from: DateScrollItem) -> [DateScrollItem] {
        ...
        print()
        print("XXXXXXXXXXXX")
        print(monthStartDate.format(dateFormat: "dd/MM/yyyy MMM EEE HH:mm:ss"))
        print("XXXXXXXXXXXX")
        print("XXXXXXXXXXXX")
        print(monthEndDate.format(dateFormat: "dd/MM/yyyy MMM EEE HH:mm:ss"))
        print("XXXXXXXXXXXX")
        var currentDate = monthStartDate
        var days = [DateScrollItem]()
        
        if format.separatorEnabled {
            days.append(DateScrollItem(date: currentDate, selected: false, hasevent: 1, separator: true))
        }
        while currentDate < monthEndDate {
            days.append(DateScrollItem(date: currentDate, selected: currentDate == Date.today(),hasevent: 1, separator: false))
    ...
    private func nextMonthDays(from: DateScrollItem) -> [DateScrollItem] {
        ...
        if format.separatorEnabled {
            days.append(DateScrollItem(date: monthEndDate, selected: false,hasevent: 1, separator: true))
        }
        return days
    }
}
